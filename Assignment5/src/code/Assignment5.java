...
public class Assignment5 {
	...
	private  JTextField bruteSearchField;
	...
	private void initialise() {
		 ...
		 * Brute Force Search
		 */
		bruteSearchField = new JTextField(SEARCH_COLS);
		bruteSearchField.setMaximumSize(new Dimension(0, 25));
		bruteSearchField.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String pattern = bruteSearchField.getText();
				String text = textEditor.getText();
				int index = new KMP(pattern, text).bruteSearch(pattern, text);
				...
				if (index == -1) {
					...
					textEditor.setSelectionStart(index);
					textEditor.setSelectionEnd(index + pattern.length());
					textEditor.setSelectionColor(Color.PINK);
				}
			}
		});
		...
		controls.add(new JLabel("Search KMP"));
		controls.add(Box.createRigidArea(new Dimension(LAYOUT_GAP, 0)));
		controls.add(searchField);

		/**
		 * Brute Search controls
		 */
		controls.add(new JLabel("Search Brute Force"));
		controls.add(bruteSearchField);
